...
public class BTree<Key extends Comparable<Key>, Value>  {
    ...
    public Key min(){
        if(this.root.children[0] == null) return null;
            else return min(this.root.children[0].next);        
    }

    public Key min(Node x){
        if(x.children[0].next == null) return (Key)x.children[0];
            else return min(x.children[0].next);
    }

    public Key max(){
        if(this.root.children[(this.root.m) - 1] == null) return null;
            else return max(this.root.children[(this.root.m) - 1].next);
    }

    public Key max(Node x){
        if(x.children[x.m-1] == null) return (Key)x.children[(x.m)-1];
            else return max(x.children[(x.m)-1].next);
    }
