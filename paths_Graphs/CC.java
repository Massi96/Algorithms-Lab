...
public class CC {
    ...
    Aggiungere un metodo che calcola il grado di separazione di
    un grafo, ovvero il numero medio di passi che occorrono per
    connettere due nodi del grafo (ovviamente appartenenti alla
    stessa componente connessa)*/

    /**/
    private int gradonodo(Graph G,int v){
        DepthFirstPaths dfs = new DepthFirstPaths(G, v);
        int tmp=0; 
        for(int i=0;i<G.V();i++){
            int path=0 ; 
            /*Se esiste un path tra v e i allora conto la lunghezza di questo*/
            if(dfs.hasPathTo(i)){
                for(Integer nodo:dfs.pathTo(i))
                    path++; 
                /*in tmp alla fine avrò la lunghezza di tutti gli altri percorsi*/
                tmp+=path-1;    
            }
        }
        /*media*/
        return tmp/(size[id(v)]-1); 
    }

    private int gradoseparazione(Graph G){
       int tmp=0; 
        int[] gradiseparazione=new int[count]; 
        for(int i=0;i<G.V();i++)
            gradiseparazione[id(i)]+=gradonodo(G,i); 
        /*media dei gradi di una stessa componente connessa*/
        for(int j=0;j<gradiseparazione.length;j++)
            gradiseparazione[j]=gradiseparazione[j]/size[j];
        for(int z=0;z<gradiseparazione.length;z++)
             tmp+=gradiseparazione[z]; 
    ...
    public static void main(String[] args) {
        ...
        StdOut.println("");
        StdOut.println("grado separazione :"+cc.gradoseparazione(G));
